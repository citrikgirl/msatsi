...
char **argv; /* argument string */
{
	...
	++argv;
	if(argc<2){
		printf("usage: satsifast_2D data_file damping\n");
		return;
	}
	fpin=fopen(*argv,"r");
	if(fpin==NULL){
	...
	for (nx=0;nx<MAXX;nx++)
	  for (ny=0;ny<MAXY;ny++)
            if ((loclist[MAXY*nx+ny]>-999)&&(loclist[MAXY*nx+ny]<nlocfill)) 
	      {
	        k=5*loclist[MAXY*nx+ny];
	        strten[0][0]= stress[k];
	        ...
	        fprintf(fpout,"%d %d %g %g %g %g %g %g\n",
                   nx,ny,strten[0][0],strten[0][1],strten[0][2],strten[1][1],strten[1][2],strten[2][2]);
 	        if(lam[0] != lam[2])
                  {
